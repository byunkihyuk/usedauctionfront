...
function AuctionTransactionSellHistory() {
    ...
    useEffect(() => {
        ...
        fetch(
            ...
            }/api/user/${userId}/auction-sell-history?size=${pageSize}&page=${
    ...
    function onClickProductUpdate(auctionTransactionId) {
        console.log(auctionTransactionId);
        movePage(`/auction/${auctionTransactionId}/updatepage`);
    }
    function onClickProductDelete(auctionTransactionId) {
    ...
    return (
        <Layouts>
            ...
            <div className="w-full h-10 flex items-center justify-between border-b-2 lg:min-w-80">
                <div className="text-lg">경매 판매 내역 페이지</div>
                ...
                <div className="grid lg:grid-cols-2 lg:gap-y-2 lg:gap-x-10 sm:grid-cols-1 md:grid-cols-2 place-items-center justify-center">
                    {data.map((data, index) => (
                        <ProductCardHistory
                            ...
                            info={data}
                            sell={true}
                            onClick={() => movePage(`/auction/${data.auctionTransactionId}`)}
                            onClickDelete={() => onClickProductDelete(data.auctionTransactionId)}
                            onClickUpdate={() => onClickProductUpdate(data.auctionTransactionId)}
                        ></ProductCardHistory>
                    ))}
                </div>
            ) : (
...
export default AuctionTransactionSellHistory;
