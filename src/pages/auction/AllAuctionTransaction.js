...
function AllAuctionTransaction() {
    ...
    const onClickTransactionStateDropDown = (state) => {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(
            `${process.env.REACT_APP_CLIENT_IP}/api/auction?size=${pageSize}&page=${
        ...
        <Layouts>
            ...
            <div className="max-w-[1000px] w-full flex flex-col items-center justify-center space-y-5">
                <div className="w-full h-10 flex items-center justify-between border-b-2">
                    <div className=" flex items-center justify-center">경매 거래 전체 글</div>
                                ...
                                >
                                    <div className="py-1" role="none">
                                        {transactionStateList.map((item, index) => (
                                            <a
                                                key={index}
                                                href={`/auction?size=${pageSize}&page=${1}&sort=${pageSort}&state=${item}&keyword=${keyword}`}
                                                className="text-gray-700 block px-4 py-1 text-sm cursor-pointer hover:bg-gray-300"
                                                role="menuitem"
                                                tabIndex="-1"
                                                id="menu-item-1"
                                                // onClick={() =>
                                                //     onClickTransactionStateDropDown(data)
                                                // }
                                            >
                                                {item}
                                            </a>
                                ...
                                >
                                    <div className="py-1" role="none">
                                        {sortList.map((item, index) => (
                                            <a
                                                key={index}
                                                href={`/auction?size=${pageSize}&page=${pageNumber}&sort=${
                ...
                >
                    {data && data.transactionList.length > 0
                        ? data.transactionList.map((item, index) => (
                              <ProductCard
                                  key={index}
                                  info={item}
                                  url={"/auction/" + item.auctionTransactionId}
                                  type={"auction"}
                              ></ProductCard>
                          ))
                        : "등록된 경매가 없습니다."}
                ...
                <div className="grid mobile:grid-cols-8 pc:grid-flow-col  ">
                    {pageNumber > 1 ? (
                        <a
                            ...
                            href={`/auction?size=${pageSize}&page=${1}&sort=${pageSort}&state=${transactionState}`}
                        >
                            {"<<"}
                        </a>
                    ) : (
                    ...
                    {start - 10 >= 0 ? (
                        <a
                            href={`/auction?size=${pageSize}&page=${
                    ...
                    {pages.map((data, index) => (
                        <a
                            key={index}
                            href={`/auction?size=${pageSize}&page=${data}&sort=${pageSort}&state=${transactionState}`}
                    ...
                    {pageNumber < totalPages ? (
                        <a
                            href={`/auction?size=${pageSize}&page=${totalPages}&sort=${pageSort}&state=${transactionState}`}
                            className="w-8 h-8 flex items-center justify-center cursor-pointer hover:bg-purple-200"
                        >
                            {">>"}
                        </a>
...
export default AllAuctionTransaction;
