...
function AuctionTransaction() {
    ...
    const deadLine = new Date() > new Date(data && data.finishedAt) ? true : false;
    ...
    function onClickBidList() {
        ...
        fetch(`${process.env.REACT_APP_CLIENT_IP}/api/auction/${auctionTransactionId}/bid/all`, {
            method: "GET",
            headers: {
                Authorization: `Bearer ${localStorage.getItem("loginToken")}`,
            },
    ...
    function onClickTrade(item) {
        if (window.confirm("거래를 진행하시겠습니까?")) {
            fetch(`${process.env.REACT_APP_CLIENT_IP}/api/pay/auction/bid/${item.auctionBidId}`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("loginToken")}`,
                    "Content-Type": "application/json",
        ...
        <Layouts>
            ...
            <div className="flex flex-col items-center justify-center space-y-4">
                <div className="flex mobile:flex-col  items-center justify-center">
                    <div className="relative pc:w-[500px] pc:h-[500px] mobile:w-96 mobile:h-80 overflow-hidden">
                        {data && data.images.length > 0 ? (
                            ...
                            <img
                                alt="noimage"
                                src={noimage}
                                className="pc:w-[500px] pc:h-[500px] mobile:w-96 mobile:h-80"
                            ></img>
                        )}
                    ...
                    <div className="relative pc:w-[500px] pc:h-[500px] mobile:w-96 mobile:h-96 flex flex-col items-start justify-between  pc:p-5 mobile:p-2">
                                ...
                                {optionDropDown && (
                                    ...
                                    >
                                        <div
                                            ...
                                            onClick={() =>
                                                movePage(
                                                    `/auction/${auctionTransactionId}/updatepage`
                                                )
                                            }
                                        >
                                            수정
                        ...
                        <div className="text-xl font-bold">
                            내 입찰가격 :{" "}
                            {Number(mybid && mybid.price ? mybid.price : 0).toLocaleString()} 원
                        </div>
                        <div>
                ...
                <div className="pc:w-[1000px] mobile:w-96 p-2 whitespace-pre text-left">
                                    ...
                                    >
                                        <div className="flex flex-col">
                                            <div>
                                                <div className="flex items-center justify-center">
                                                    <div
                                                        title={item.bidderNickname}
                                                        className="w-20"
                                                    >
                                                        {item.bidderNickname}
                                                    </div>
                                                    <div
                                                        title={item.auctionBidState}
                                                        className="w-20"
                                                    >
                                                        {item.price}
                                                    </div>
                    ...
                    <div className="absolute w-96 h-60 bg-white rounded-lg flex flex-col">
                        ...
                        <div className="flex m-2 items-center justify-center space-x-2">
                            {confirmation ? (
                                ...
                                <CustomButton text={"입찰"} onClick={onClickBidding}></CustomButton>
                            )}
                            <CustomButton
                                text={"닫기"}
                                onClick={() => setBidderModal(false)}
